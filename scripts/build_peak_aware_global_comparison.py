#!/usr/bin/env python3
"""Build Stage 3 Task 4 comparison tables and plots for Peak-Aware Global GRU."""
...
COLOR_TREATMENT = "#C44E52"
COLOR_ACTUAL = "#2CA02C"

PEAK_SUBSET_TREATMENT_MODEL = "peak_aware_global"
PEAK_SUBSET_BASELINE_MODEL = "baseline"
...
def _parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Build comparison tables and plots for Peak-Aware Global evaluation.",
    )
    parser.add_argument(
        "--experiment-dir",
        type=Path,
...
def build_comparison_table(
    ...
    baseline_peak = peak_subset_summary.loc[
        peak_subset_summary["model"] == PEAK_SUBSET_BASELINE_MODEL
    ].iloc[0]
    treatment_peak = peak_subset_summary.loc[
        peak_subset_summary["model"] == PEAK_SUBSET_TREATMENT_MODEL
    ].iloc[0]
    ...
    rows.append(
        {
            ...
            "peak_aware_global": float(treatment_peak["peak_step_fraction"]),
...
def build_per_container_comparison(
    treatment_df: pd.DataFrame,
    baseline_df: pd.DataFrame,
    treatment_peak_df: pd.DataFrame,
    baseline_peak_df: pd.DataFrame,
) -> pd.DataFrame:
    """Primary Stage 4 artifact — paired per-container overall + peak metrics."""
    overall = treatment_df.merge(
        baseline_df,
        on="container_id",
        suffixes=("_treatment", "_baseline"),
    )
    peak = treatment_peak_df.merge(
        baseline_peak_df,
        on="container_id",
        suffixes=("_treatment", "_baseline"),
    )

    merged = overall.merge(peak, on="container_id", suffixes=("", "_peak"))

    comparison = pd.DataFrame(
        {
            "container_id": merged["container_id"],
            "baseline_day1_mae": merged["day1_mae_baseline"],
            "baseline_day1_rmse": merged["day1_rmse_baseline"],
            "baseline_day1_mape": merged["day1_mape_baseline"],
            "treatment_day1_mae": merged["day1_mae_treatment"],
            "treatment_day1_rmse": merged["day1_rmse_treatment"],
            "treatment_day1_mape": merged["day1_mape_treatment"],
            "baseline_peak_mae": merged["peak_mae_baseline"],
            "baseline_peak_rmse": merged["peak_rmse_baseline"],
            "treatment_peak_mae": merged["peak_mae_treatment"],
            "treatment_peak_rmse": merged["peak_rmse_treatment"],
            "baseline_non_peak_mae": merged["non_peak_mae_baseline"],
            "baseline_non_peak_rmse": merged["non_peak_rmse_baseline"],
            "treatment_non_peak_mae": merged["non_peak_mae_treatment"],
            "treatment_non_peak_rmse": merged["non_peak_rmse_treatment"],
        }
    )

    for metric in ("day1_mae", "day1_rmse", "day1_mape"):
        comparison[f"delta_{metric}"] = (
            comparison[f"treatment_{metric}"] - comparison[f"baseline_{metric}"]
        )
    ...
    return comparison[list(PER_CONTAINER_COLUMNS)].sort_values(
        "container_id"
...
def build_evaluation_summary_json(
    ...
    per_container_comparison: pd.DataFrame,
    experiment_dir: Path,
) -> dict[str, object]:
    """Machine-readable comparison record for Stage 3 lock."""
    ...
    return {
        ...
        "containers_evaluated": int(len(per_container_comparison)),
        "overall_metrics": {
            row["metric"]: {
                "baseline": row["baseline"],
                "peak_aware_global": row["peak_aware_global"],
        ...
        "per_container_delta_summary": {
            "delta_day1_mae": {
                "mean": float(per_container_comparison["delta_day1_mae"].mean()),
                "std": float(per_container_comparison["delta_day1_mae"].std()),
                "min": float(per_container_comparison["delta_day1_mae"].min()),
                "max": float(per_container_comparison["delta_day1_mae"].max()),
                "containers_improved": int(
                    (per_container_comparison["delta_day1_mae"] < 0).sum()
                ),
            ...
            "delta_day1_rmse": {
                "mean": float(per_container_comparison["delta_day1_rmse"].mean()),
                "std": float(per_container_comparison["delta_day1_rmse"].std()),
                "min": float(per_container_comparison["delta_day1_rmse"].min()),
                "max": float(per_container_comparison["delta_day1_rmse"].max()),
            },
            "delta_peak_mae": {
                "mean": float(
                    per_container_comparison["delta_peak_mae"].dropna().mean()
                ),
                "containers_peak_improved": int(
                    (per_container_comparison["delta_peak_mae"] < 0).sum()
                ),
            },
        },
        "outputs": {
            "comparison_table": "evaluation/comparison_table.csv",
            "per_container_comparison": "evaluation/per_container_comparison.csv",
...
def _select_plot_containers(per_container_comparison: pd.DataFrame) -> list[str]:
    """Pick representative containers for actual-vs-predicted panels."""
    valid = per_container_comparison.dropna(subset=["delta_day1_mae"])
    median_idx = (
        valid["delta_day1_mae"] - valid["delta_day1_mae"].median()
    ).abs()
...
def plot_actual_vs_predicted(
    ...
    for ax, container_id in zip(axes, container_ids):
        ...
        treatment = treatment_cache[container_id][GLOBAL_INFERENCE_CACHE_PRED_KEY]
        baseline = baseline_cache[container_id][GLOBAL_INFERENCE_CACHE_PRED_KEY]
    ...
    fig.suptitle(
        "Actual vs Predicted — Sample Containers (Global Track)",
        fontsize=13,
        y=1.01,
    )
    fig.tight_layout()
...
def plot_error_distribution(
    ...
    deltas = per_container_comparison["delta_day1_mae"].dropna()
    ...
    ax.set_title(
        "Per-Container Day-1 MAE Delta (Peak-Aware Global − Global GRU)",
        fontsize=12,
    )
...
def plot_peak_subset_comparison(
    ...
    baseline_vals = [
        float(subset.loc[subset["metric"] == m, "baseline"].iloc[0])
        for m in metric_keys
    ]
    treatment_vals = [
        float(subset.loc[subset["metric"] == m, "peak_aware_global"].iloc[0])
        for m in metric_keys
    ]
    ...
    ax.set_title(
        "Pooled Peak-Subset Metrics — Global GRU vs Peak-Aware Global",
        fontsize=12,
    )
    ax.set_xticks(x)
...
def main() -> None:
    ...
    print("=" * 72)
    print("Peak-Aware Global — Build Comparison Tables + Plots (Task 4)")
    print("=" * 72)
    ...
    treatment_peak_df = pd.read_csv(treatment_peak_path)
    baseline_peak_df = pd.read_csv(baseline_peak_path)
    ...
    per_container_comparison = build_per_container_comparison(
        treatment_df,
        baseline_df,
        treatment_peak_df,
        baseline_peak_df,
    ...
    evaluation_summary = build_evaluation_summary_json(
        comparison_table,
        per_container_comparison,
        experiment_dir,
    )

    comparison_path = evaluation_dir / "comparison_table.csv"
    per_container_path = evaluation_dir / "per_container_comparison.csv"
    summary_json_path = evaluation_dir / "evaluation_summary.json"

    comparison_table.to_csv(comparison_path, index=False)
    per_container_comparison.to_csv(per_container_path, index=False)
    ...
    sample_containers = _select_plot_containers(per_container_comparison)
    ...
    plot_paths.extend(plot_error_distribution(per_container_comparison, plots_dir))
    ...
    print()
    print("Overall (frozen baseline reference vs peak-aware global):")
    for metric in ("day1_mae", "day1_rmse", "day1_mape"):
        ...
        print(
            ...
            f"treatment={row['peak_aware_global']:.6f}  "
            f"delta={row['delta_treatment_minus_baseline']:+.6f}"
        )

    print()
    print(
        "Baseline re-run MAE (paired control):",
        f"{baseline_rerun_summary['day1_mae']:.6f}",
    )
    print(
        "Per-container ΔMAE: mean =",
        f"{per_container_comparison['delta_day1_mae'].mean():+.6f}",
    )
    print(f"Per-container rows: {len(per_container_comparison)}")
    print()
    print("Saved:")
    print(f"  {comparison_path}")
    print(f"  {per_container_path}")
